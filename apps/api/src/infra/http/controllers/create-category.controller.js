"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.CreateCategoryController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const zod_1 = require("zod");
const create_category_1 = require("../../../domain/restaurant/application/use-cases/create-category");
const conflict_exception_error_1 = require("../../../domain/restaurant/application/use-cases/errors/conflict-exception-error");
const roles_decorator_1 = require("../../auth/roles-decorator");
const roles_enum_1 = require("../../auth/roles-enum");
const zod_validation_pipe_1 = require("../pipes/zod-validation-pipe");
const category_presenter_1 = require("../presenters/category-presenter");
const createCategoryBodySchema = zod_1.z.object({
    name: zod_1.z.string(),
});
const bodyValidationPipe = new zod_validation_pipe_1.ZodValidationPipe(createCategoryBodySchema);
let CreateCategoryController = class CreateCategoryController {
    createCategory;
    constructor(createCategory) {
        this.createCategory = createCategory;
    }
    async handle(body) {
        const { name } = body;
        const result = await this.createCategory.execute({
            name,
        });
        if (result.isLeft()) {
            const error = result.value;
            switch (error.constructor) {
                case conflict_exception_error_1.ConflictExceptionError:
                    throw new common_1.BadRequestException(error.message);
                default:
            }
        }
        const category = result.isRight() ? result.value.category : null;
        if (!category) {
            throw new common_1.BadRequestException('Something went wrong');
        }
        return {
            category: category_presenter_1.CategoryPresenter.toHTTP(category),
        };
    }
};
exports.CreateCategoryController = CreateCategoryController;
__decorate([
    (0, roles_decorator_1.Roles)(roles_enum_1.Role.ADMIN),
    (0, common_1.Post)(),
    __param(0, (0, common_1.Body)(bodyValidationPipe)),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], CreateCategoryController.prototype, "handle", null);
exports.CreateCategoryController = CreateCategoryController = __decorate([
    (0, swagger_1.ApiTags)('Categories'),
    (0, common_1.Controller)('/categories'),
    __metadata("design:paramtypes", [create_category_1.CreateCategoryUseCase])
], CreateCategoryController);
//# sourceMappingURL=create-category.controller.js.map